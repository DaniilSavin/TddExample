...
LinearEquation::LinearEquation(string _s)
{
	...
	for (int i = 0; i < s.size(); i++)
		if (s[i] != "") 
			coefficients.push_back(stod(s[i].c_str()));
}
...
{
        for_each(coefficients.begin(), coefficients.end(), [](double& t) 
		{
		t = (rand() % 100) / 10.0; 
		});
}
...
{
	for_each(coefficients.begin(), coefficients.end(), [value](double& t)
		{
			t = value;
		});
}
...
{
	vector<double> res = coefficients;
	for_each(res.begin(), res.end(), [](double& t)
		{
			t = -t;
		});
	return LinearEquation(res);
